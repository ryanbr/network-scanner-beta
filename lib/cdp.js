...
async function createCDPSession(page, currentUrl, options = {}) {
  ...
  try {
    // Enhanced connection reliability for Puppeteer 24.x
    // Add timeout wrapper for CDP session creation
    const sessionPromise = page.target().createCDPSession();
    cdpSession = await Promise.race([
      sessionPromise,
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('CDP session creation timeout')), 15000))
    ]);
    // Enable network domain with timeout for 24.x stability
    await Promise.race([
    ...
    return {
      ...
      cleanup: async () => {
        ...
        if (cdpSession) {
          try {
           // Enhanced detach process for 24.x compatibility
            try {
              // Try graceful detach first
              await Promise.race([
                cdpSession.detach(),
                new Promise((_, reject) => 
                  setTimeout(() => reject(new Error('CDP detach timeout')), 8000))
              ]);
            } catch (detachErr) {
              // If graceful detach fails, force close the connection
              if (detachErr.message.includes('timeout')) {
                // Force cleanup - this is expected in some 24.x scenarios
                cdpSession = null;
                if (forceDebug) {
                  console.log(formatLogMessage('debug', `CDP session force-closed for ${currentUrl} (timeout expected in 24.x)`));
                }
            ...
            // Handle timeout errors more gracefully in Puppeteer 24.x
            if (cdpCleanupErr.message.includes('CDP detach timeout') || 
                cdpCleanupErr.message.includes('Target.detachFromTarget timed out')) {
              if (forceDebug) {
                console.log(formatLogMessage('debug', `CDP session detach timed out for ${currentUrl} - this is expected with Puppeteer 24.x`));
              }
            } else if (cdpCleanupErr.message.includes('Session closed') || 
                       cdpCleanupErr.message.includes('Connection closed')) {
              // New 24.x error types - treat as normal cleanup
              if (forceDebug) {
                console.log(formatLogMessage('debug', `CDP session already closed for ${currentUrl} - normal in 24.x`));
              }
            } else {
              // Log other cleanup errors but don't throw
              if (forceDebug) {
...
async function createEnhancedCDPSession(page, currentUrl, options = {}) {
  ...
  try {
    ...
    // Add timeouts for 24.x stability
    await Promise.race([
      session.send('Runtime.enable'),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Runtime.enable timeout')), 10000))
    ]);
    
    await Promise.race([
      session.send('Security.enable'),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Security.enable timeout')), 10000))
    ]);
