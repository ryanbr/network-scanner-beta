...
async function gracefulBrowserCleanup(browser, forceDebug = false) {
      ...
      try {
        ...
        await Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Cleanup page close timeout')), 8000)
          )
        ]);
        if (forceDebug) console.log(`[debug] [browser] Page closed successfully`);
      } catch (err) {
        // Handle timeout gracefully - don't fail cleanup on timeouts
        if (err.message.includes('Target.closeTarget timed out') ||
            err.message.includes('Cleanup page close timeout')) {
          if (forceDebug) console.log(`[debug] [browser] Page close timed out - this is expected with Puppeteer 24.x`);
        } else {
  ...
  // Add timeout wrapper for browser.close() in Puppeteer 24.x
  try {
    await Promise.race([
      browser.close(),
      new Promise((_, reject) => 
    ...
    if (browserCloseErr.message.includes('Browser close timeout') ||
        browserCloseErr.message.includes('Target.closeTarget timed out')) {
      if (forceDebug) console.log(`[debug] [browser] Browser close timed out - proceeding with force cleanup`);
      // Don't throw - let force cleanup handle it
    } else {
      throw browserCloseErr; // Re-throw non-timeout errors
    }
  }
  ...
  try {
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => {
          // Use longer timeout for Puppeteer 24.x
          const adjustedTimeout = Math.max(timeout, 15000); // Minimum 15 seconds
          setTimeout(() => reject(new Error('Browser cleanup timeout')), adjustedTimeout);
        }
        )
      ]);
      ...
      // Don't treat timeout errors as critical failures in Puppeteer 24.x
      if (browserCloseErr.message.includes('Browser cleanup timeout') ||
          browserCloseErr.message.includes('Target.closeTarget timed out')) {
        if (forceDebug) console.log(`[debug] [browser] Browser cleanup timed out - this is expected with Puppeteer 24.x`);
        results.errors.push(`Browser cleanup timed out (expected in Puppeteer 24.x)`);
      } else {
